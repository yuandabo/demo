...
<template>
  <div class="typical-overrun-event">
    <ModuleTitle :title="module.name" :dateForm="dateForm" :dateShow="true">
        ...
        <template>
          ...
          <div class="show-all" v-if="eventList.length > lengthLimit" ref="put">
            <div class="btn" @click="switchAll">
              <i
                class="reda-icon"
                :class="isOpen ? 'ascending' : 'descending'"
              ></i>
...
export default class extends Vue {
  ...
  poChange (): void {
    if (this.isOpen) {
      const bodyHeight: number = (document.querySelector(
        'body'
      ) as HTMLElement).getBoundingClientRect().bottom
      const { bottom: buttonHeight, top: buttonTop } = (this.$refs
        .put as HTMLElement).getBoundingClientRect()
      const { top: eventTop } = (this.$refs
        .event as HTMLElement).getBoundingClientRect()
      if (
        bodyHeight <= buttonTop &&
        bodyHeight <= buttonHeight &&
        !this.showSticky
      ) this.$emit('show-sticky', true)
