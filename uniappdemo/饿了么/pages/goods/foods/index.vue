<template>
  <div class="flexdir-column foods">
    <div class="goods-img">
      <image :src="'https://cube.elemecdn.com/2/14/7059f0fe8c3eb2c691a595ef7f734png.png?x-oss-process=image/format,webp/resize,w_686'">
      </image>
    </div>
    <div v-show="showCom"
...
import shoperrecommend from '@/pages/shoperRecommend'
...
export default {
  ...
  computed: {
    ...
    FOODSLENGTHLIST () {
      ...
      this.goods.forEach((v, index) => {
        let val = 0
        if (index === 0) {
          val = v.foods.length * 131 + 26
        } else {
          val = v.foods.length * 131 + arr[index] + 26
        }
        arr.push(val)
      })
      return arr
  ...
  methods: {
    ...
    foodDec ($event) {
      store.commit('app/shopcar/changeOneData', {
        id: $event.id,
        numberSize: -1
      })
    },
