	...
	<view class="about">
		<view class="flex-item flex-item-V about-search">
			<view class="uni-flex uni-row search-warpper">
				...
				<input class="flex-item search search-box" ref="input" type="number" value="" v-model="inputValue" placeholder="在这输入你的持仓.例(1000)" />
				<view class="icon">
					<uni-icons class="icon" type="plus" @click="searchClick" size="30" />
				</view>
				<view class="icon">
					<uni-icons class="icon" type="refresh-filled" size="30" @click="getSharesData" />
				</view>
			</view>
		</view>
		<view class="about-card">
			<uni-card title="今日收益情况" :extra="`本金：${currentMoney}元`">
				...
				<view class="about-content" :style="{color:getOrOut>0?'red':'green'}">
			...
			<view class="qiun-charts" v-show="active===0&&showcharts">
				<canvas style="z-index: 10;" canvas-id="canvasLineA" id="canvasLineA" class="charts" @touchstart="touchLineA"></canvas>
			</view>
			<image v-if="active===1" class="about-img" :src="`http://j4.dfcfw.com/charts/pic6/${code}.png`"></image>
		</view>
		<view class="button-check">
			<button class="checkBut" @click="selectToMy">
				{{isSelectWord}}
			</button>
			<button class="checkBut2" type="default" @click="open()">{{name}}重仓持股</button>
		</view>

		<view class="had" v-show="show">
			<view class="had-mes">
				{{name}}持仓详情:
			</view>
			...
			<view class="uni-flex uni-row had-list" :style="{color:item.ChangeRatio>0?'red':'green'}" v-for="(item) in Holds"
			 :key="item.CYGP">
			...
			<button type="default" class="button-close" @click="closeHad">关闭</button>
		</view>
	</view>
</template>
